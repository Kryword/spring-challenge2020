...
func TestPacCreation(t *testing.T) {
	...
	pac := new(game.Pac)
	pac.Init(expId, expX, expY, expType, expCooldown, expSpeedTurns, false)
	if pac.Id != expId || pac.Pos.X != expX || pac.Pos.Y != expY || pac.PacType != expType || pac.Cooldown != expCooldown || pac.SpeedTurns != expSpeedTurns || pac.Mine {
		t.Errorf("Pac not created correctly %+v different to expected [%d %d %d %d %d %d %t]", pac, expId, expX, expY, expType, expCooldown, expSpeedTurns, false)
	}
}
